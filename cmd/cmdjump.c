...
#define SZ_FILE_BUF		1024	// 파일 I/O시 필요한 배열
...
get_argv_optv(char *cmd_line)
{
	...
	for (; (tok = strtok(NULL, " \t\n")) != NULL; ) { //다음 토큰 잘라 냄
		...
		if (tok[0] == '-')		//토큰이 옵션임을 의미. 예) optv[0]=="-s"
			...
			argv[argc++] = tok; //예) argv[0] == "file1", argv[1]=="file2"

	}
	return(cmd);
	//이 함수가 리턴해도 각 문자열은 여전히 cmd_line[]에 저장되어 있고
	//각 문자열의 시작주소만 cmd, optv[], argv[]에 저장되어 있
}
...
print_attr(char *path, char *fn)
{
	...
	buf[0] = c;
	buf[1] = (st_buf.st_mode & S_IRUSR)? 'r': '_';
	buf[2] = (st_buf.st_mode & S_IWUSR)? 'w': '_';
	buf[3] = (st_buf.st_mode & S_IXUSR)? 'x': '_';
	buf[4] = (st_buf.st_mode & S_IRGRP)? 'r': '_';
	buf[5] = (st_buf.st_mode & S_IWGRP)? 'w': '_';
	buf[6] = (st_buf.st_mode & S_IXGRP)? 'x': '_';
	buf[7] = (st_buf.st_mode & S_IROTH)? 'r': '_';
	buf[8] = (st_buf.st_mode & S_IWOTH)? 'w': '_';
	buf[9] = (st_buf.st_mode & S_IXOTH)? 'x': '_';
	buf[10] = '\0';
	pwp = getpwuid(st_buf.st_uid);
	grp = getgrgid(st_buf.st_gid);
	tmp = localtime(&st_buf.st_mtime);
	strftime(time_buf, 13, "%b %d %H:%M", tmp);
	//아래의 " "안의 l은 모두 소문자 L임' 모든 % 개수 정확히 입력할 것
...
// ":S -;" 옵션 준 경우: 디렉토리의 모든 파일에 대해 상세정보 출력하기 
static void 
...
//			이를 기준으로 한줄 에 출력할 수 있는 열(cplumn)의 개수를 결정함

static void
get_max_name_len(DIR *dp, int *p_max_name_len, int *p_num_per_line)
{
		struct dirent *dirp;
		int max_name_len = 0; //가장 긴 파일이름의 길이
...
//"ls"만한 경우: 디렉토리의 모든 파일에 대해 이름만 출력하기
static void
print_name(DIR *dp)
{
		struct dirent *dirp;
...
gethostname(hostname, SZ_STR_BUF);  //컴퓨터 이름을 구해와, hostname[]에 저장
...
printf("id: 잘못된 사용자 이름: \"%s\"\n",argv[0]);
	...
	printf("uid=%d(%s) gid=%d(%s) 그룹들=%d(%s)\n",pwp->pw_uid, pwp->pw_name,pwp->pw_gid,grp->gr_name,grp->gr_gid,grp->gr_name);
...
if (mkdir(argv[0], 0755) < 0)
	...
	//읽고(r: ls 실행가능), 쓰고(w: 파일 생성 및 삭제 가능),
...
// 그러나 help() 함수의 정의는 cmd_tbl[] 뒤에 있기 때문에, cmd_tbl[] 앞에 
// 이 함수를 아래처럼 미리 선언해야 한다. help() 함수정의는 cmd_tbl[]을 
// 직접 사용하기 때문에 어쩔 수 없이 cmd_tbl[] 뒤에 있어
...
//에러가 발생했으면 에러 원인 출력하고 리턴함
	...
	if ((pid = fork()) < 0)
		longjmp(jump,-1);
//이 순간부터 동일한 프로그램(cmd) 2개가 동시에 실행되고 있으며,
//프로그램 내에서 실행되는 위치도 바로 이 위치에서 계속 실행된다.

//fork()의 리턴 값(pid)이 0이면 자식 프로세스이고, 0보다 크면 부모 프로세스임
...
//이 과정에서 에러가 발생했으면 에러 원인 출력하고 스스로 종료한다.
	else if (pid == 0) //자식 프로세스
	{
		...
		//순서대로 nargv[]에 연속적으로 저장함' nargv[]의 마지막은 NULL이어야함
		...
		//해당 프로그램이나 접근 권한이 없거나, 또는 실행 프로그램이 아닐경우 종료
		{
			perror(cmd);		//PRINT_ERR_RET 못쓴다.
			exit(1);
		}
	}			//자식 프로세스는 외부 프로그램으로 대체되었기 때문에 이후의 코드는 실행되지 않는다.




//fork()의 리턴 값(pid)이 양수(pid=자식 프로세스의 id)이면 부모 프로세스임
//부모 프로세스(cmd 프로그램)은 자식 프로세스(외부 프로그램)가 종료될때까지 더 이상 실행되지 않고 여기서 대기해야함.
//이 과정에서 에러가 발생했으면 에러 원인 출력하고 스스로 종료한다.
	else {
	//pid>0인경우, 즉 부모 프로세스
	//자식(pid)이 종료될 때까지 대기
...
proc_cmd(void)
{
	...
	for (k = 0; k < num_cmd; ++k) {	// 입력한 명령어 정보를 cmd_tbl[]에서 찾음
		...
		if (EQUAL(cmd, cmd_tbl[k].cmd)) { // 입력 명령어를 cmd_tbl에서 찾았음
			...
			cmd_idx = k;
			check_arg(cmd_tbl[k].argc); 	// 인자 개수 체크
			check_opt(cmd_tbl[k].opt);		// 옵션 체크
...
main(int argc, char *argv[])
{
	...
	for (;;) {
		printf("<%s> %d:  ",cur_work_dir, cmd_count);		// 명령 프롬프트: "$ "
